...
The scraper downloads a single OFX file covering all transactions in the current calendar year and writes it to `output_directory`.

<Note>
  The `finance_dl.discover` web scraper also enforces a 5-second delay between requests to comply with Discover's rate limits. This delay is built in and requires no configuration.
</Note>
